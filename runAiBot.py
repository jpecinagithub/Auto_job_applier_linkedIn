...
from selenium.common.exceptions import NoSuchElementException, ElementClickInterceptedException, NoSuchWindowException, ElementNotInteractableException, TimeoutException, WebDriverException, StaleElementReferenceException
...
def click_filter_text(container: WebElement, label: str, click_gap_buffer: bool = True) -> bool:
    ...
    for _ in range(3):
        try:
            candidates = container.find_elements(By.XPATH, ".//*[self::span or self::button or self::label or self::div or self::input]")
            for element in candidates:
                try:
                    ...
                    continue
                normalized_probes = [normalize_filter_text(item) for item in probes if item]
...
def multi_sel_flexible(container: WebElement, texts: list[str], actions: ActionChains = None) -> None:
    for text in texts:
...
def boolean_button_click_flexible(container: WebElement, actions: ActionChains, text: str) -> None:
    ...
    for _ in range(3):
        try:
            candidates = container.find_elements(By.XPATH, ".//*[self::label or self::span or self::div or self::button]")
            for candidate in candidates:
                try:
                    candidate_text = normalize_filter_text(candidate.text or candidate.get_attribute("aria-label") or "")
                except StaleElementReferenceException:
                    continue
                if not candidate_text or not any(lbl == candidate_text or lbl in candidate_text for lbl in label_norms):
                    continue
                try:
                    row = candidate.find_element(By.XPATH, "./ancestor::*[self::li or self::div or self::fieldset or self::label][1]")
                except Exception:
                    row = candidate
                try:
                    switch = row.find_element(By.XPATH, ".//input[@role='switch' or @type='checkbox']")
...
def is_logged_in_LN() -> bool:
    ...
    * Returns: `True` if user is logged-in or `False` if not
    ...
    if try_linkText(driver, "Join now") or try_linkText(driver, "Únete ahora"):
        return False
    if try_xp(driver, '//button[@type="submit" and (contains(., "Sign in") or contains(., "Iniciar sesión"))]', False):
        return False

    # Typical profile controls visible => likely logged in
    if try_xp(driver, '//button[contains(@aria-label, "Me") or contains(@aria-label, "Yo") or contains(@aria-label, "Perfil")]', False):
        return True

    print_lg("Login status is unclear. Treating as not logged in to avoid false positives.")
    return False
    ...
    * If both failed, asks user to login manually
    ...
    try:
        ...
        try:
            ...
            try:
                user_input = driver.find_element(By.XPATH, '//input[@name="session_key"]')
        ...
        try:
            ...
            try:
                pass_input = driver.find_element(By.XPATH, '//input[@name="session_password"]')
    ...
    try:
        ...
        wait.until(lambda d: "/feed" in d.current_url.lower() or "/jobs" in d.current_url.lower() or "/checkpoint" in d.current_url.lower())
        if is_logged_in_LN():
            return print_lg("Login successful!")
        print_lg("LinkedIn requested extra verification. Complete it manually.")
    except Exception:
        print_lg("Seems like login attempt failed! Possibly due to wrong credentials, captcha, or verification challenge. Try logging in manually!")
    manual_login_retry(is_logged_in_LN, 2)
#>
    ...
    if search_location.strip():
        try:
            ...
            if not search_location_ele:
                search_location_ele = try_xp(driver, "(//input[contains(@class,'jobs-search-box__text-input') and not(@disabled)])[2]", False)
        ...
        for attempt in range(3):
            try:
                if not open_all_filters():
                    print_lg("Couldn't find 'All filters' button. Continuing without applying custom filters.")
                    return
                ...
                if under_10_applicants: boolean_button_click_flexible(driver, actions, "Under 10 applicants")
                if in_your_network: boolean_button_click_flexible(driver, actions, "In your network")
                if fair_chance_employer: boolean_button_click_flexible(driver, actions, "Fair Chance Employer")
                ...
                print_lg(f"Filters UI re-rendered during apply (attempt {attempt+1}/3). Retrying...")
                try:
                    actions.send_keys(Keys.ESCAPE).perform()
                except Exception:
                    pass
...
def click_job_card_with_retry(job: WebElement, job_id: str, title: str, company: str) -> None:
    for _ in range(3):
        try:
            job_details_button = job.find_element(By.XPATH, ".//a[contains(@href,'/jobs/view/')][1]")
        except Exception:
            try:
        ...
        try:
            ...
            buffer(0.3)
            continue
    raise WebDriverException(f'Failed to click "{title} | {company}" job. Job ID: {job_id}')


def get_job_main_details(job: WebElement, blacklisted_companies: set, rejected_jobs: set) -> tuple[str, str, str, str, str, bool, str]:
    '''
    ...
    * job_link: URL of this job if available
    ...
    if not title:
        try:
            title = job.find_element(By.XPATH, ".//*[contains(@class,'job-card-list__title')][1]").text.strip()
        except Exception:
            title = "Unknown Title"
    ...
    try:
        ...
        try:
            company = job.find_element(By.XPATH, ".//*[contains(@class,'job-card-container__primary-description')][1]").text.strip() or company
        except Exception:
            pass
        try:
            work_location = job.find_element(By.XPATH, ".//*[contains(@class,'job-card-container__metadata-item')][1]").text.strip() or work_location
        except Exception:
            pass

    if "(" in work_location and ")" in work_location and work_location.rfind("(") < work_location.rfind(")"):
        work_style = work_location[work_location.rfind('(')+1:work_location.rfind(')')].strip() or work_style
        work_location = work_location[:work_location.rfind('(')].strip() or work_location
...
def is_english_job_text(text: str) -> bool:
    ...
    english_markers = {
        "the", "and", "with", "for", "you", "your", "will", "this", "that", "from",
        "experience", "skills", "required", "apply", "job", "work", "team", "role",
    }
    non_english_markers = {
        "de", "la", "el", "los", "las", "con", "para", "que", "una", "un", "en",
        "y", "por", "se", "del", "les", "des", "und", "mit", "der", "die", "das",
        "para", "com", "uma", "gli", "che", "een", "van", "het",
    }
...
def answer_questions(modal: WebElement, questions_list: set, work_location: str, job_description: str | None = None ) -> set:
    ...
    for Question in all_questions:
        ...
        if select:
            ...
            should_answer = overwrite_previous_answers or selected_option == "Select an option"
            if is_phone_code_question and phone_country_code not in (selected_option or ""):
                should_answer = True
            if should_answer:
                ...
                try: 
                    if is_phone_code_question:
                        ...
                        if selected is None:
                            raise NoSuchElementException(f"Unable to match country code '{answer}'")
                        answer = selected
                    else:
        ...
        try:
            while current_count < switch_number:
                ...
                for index in range(len(job_listings)):
                    ...
                    job_id,title,company,work_location,work_style,skip,job_link = get_job_main_details(job, blacklisted_companies, rejected_jobs)
                    
                    if skip: continue
                    if easy_apply_only and not has_easy_apply_button():
                        print_lg(f'Skipping non-Easy Apply job while easy_apply_only=True: "{title} | {company}". Job ID: {job_id}')
                        skip_count += 1
                        continue
                    ...
                    if english_only_jobs:
                        ...
                        if not is_english_job_text(language_probe):
                            ...
                            message = "Job content doesn't look English. Skipping this job!"
                            print_lg(message)
                            failed_job(job_id, job_link, resume, date_listed, reason, message, "Skipped", screenshot_name)
                            rejected_jobs.add(job_id)
                            skip_count += 1
                ...
                if pagination_element == None:
                    print_lg("Single-page results detected (no pagination). Finishing this search term.")
                    break
                try:
            ...
            print_lg("Timed out while waiting for job listings on this search term. Moving to next term.", repr(e))
            continue
